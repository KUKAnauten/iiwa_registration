...
//_________________________________________________________________
  ...
  void buttonEventCallback(const std_msgs::String::ConstPtr& msg) {
    if(msg->data == "pose_get_pressed") {
      ...
      //first target frame: its x-axis is defined by the needleinsertion paths directional vector between entrypoint(E) and targetpoint(T) in dummy coordinates
...
// All planning according to tool_link_ee -> ther is an y_offset = z_offset = 0.0075 to where the tool holds the needle !!!

  //position only: working! -> this is what is needed to move to a pose above the dummy, with the right insertion orientation from "target_frame_1".
